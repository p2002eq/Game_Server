...
void TaskManager::SendTaskSelectorNew(Client *c, Mob *mob, int TaskCount, int *TaskList) {
	...
	for(int i=0; i<TaskCount;i++) { // max 40
		...
		outapp->WriteString("Text3 Test");	// max length 128		outapp->WriteString(StartZone);			// Zone number in ascii max length 64, probably can be separated by ; too, haven't found it used
	}
