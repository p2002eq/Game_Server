...
void LoginServer::ProcessUsertoWorldReq(uint16_t opcode, EQ::Net::Packet &p) {
	...
	if ((int32) numplayers >= x && x != -1 && x != 255 && status < 80)
		...
		Log(Logs::General, Logs::World_Server, "Account Info: Session Restricted for %i : %i ", utwr->lsaccountid,
			utwr->IPAddr);
		utwrs->response = -4;
	} else if (!client_list.CheckCLEIP(inet_addr(utwr->IPAddr))) {
		Log(Logs::General, Logs::World_Server, "Account Info: IP Restricted for %i : %i ", utwr->lsaccountid,
			utwr->IPAddr);
		utwrs->response = -5;
	} else {
